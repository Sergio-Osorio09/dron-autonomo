...
def run(profile, cond, levels, flights, seed, precision=True):
    res, times, land_err, est_err, vmax, energy = Counter(), [], [], [], [], []
    ...
    for level in levels:
        for i in range(flights):
            s = Simulation(profile=profile, level=level, seed=seed + i, wind_dir=(i * 97) % 360,
                           precision_landing=precision, **cfg)
            errs = []
            while not s.done:
                s.step(0.5)
...
def main():
    ...
    a = ap.parse_args()
    t0 = time.time()
    rows = ["| perfil | condición | éxito | fuera de la plataforma | choca | tiempo medio | error de aterrizaje "
            "| error de estimación | vel. máx. | batería usada |", "|---|---|---|---|---|---|---|---|---|---|"]
    for prof in a.profiles.split(","):
        for cond in [list(CONDITIONS)[int(i)] for i in a.conditions.split(",")]:
            r = run(prof, cond, list(LEVELS), a.flights, a.seed, not a.no_precision)
            rows.append("| %s | %s | %.0f%% | %.0f%% | %.0f%% | %s | %s | %.2f m | %.1f m/s | %.2f%% |" % (
                prof, cond, 100 * r["success"], 100 * r["missed"], 100 * r["crash"],
                "%.1f s" % r["time"] if r["time"] else "—", "%.2f m" % r["land"] if r["land"] is not None else "—",
    ...
    if a.md:
        with open(a.md, "w", encoding="utf-8") as f:
            f.write("# Resultados de la fase 1\n\n" + "\n".join(rows) + "\n" + note)
